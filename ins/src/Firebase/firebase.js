...
class Firebase {
    ...
    retrieveAllCommentsFromPost = (uid, pid) => {
        ...
        */
        ...
        // FIXME : There is a difference between "on" and "once" !!! 
        this.db.ref('user-posts/'+uid+'/'+pid+'/text').on('value', (snapshot) => { // FIXME : here if we use once, it would only retieve the comments one time, until you REFRESH the PAGE
            snapshot.forEach((item) => {
